...
class Mahasiswa:
  ...
  def getProfileMahasiswa(self, parent):
    ...
    profile_label = tk.Label(top, text=profile_info, font=('Arial', 18), anchor='w', justify="left")
    profile_label.pack()
...
def addData():
  mahasiswa = Mahasiswa(*mahasiswaInput())
...
def seeData():
  ...
  for numberMahasiswa, mahasiswa in enumerate(mahasiswa_list):
    tk.Button(data_frame, text=f"Data Mahasiswa {numberMahasiswa+1}", command=lambda m=mahasiswa: m.getProfileMahasiswa(root)).pack()
